...
/// Given a tensor of shape `l_1 ... l_k`
/// create a tensor of the same shape
/// where `l_a == l_b` do the summation
/// of `\sum_{j=1}^{l_a}` with those particular
/// entries set.
...
pub(crate) mod test {
    ...
    use crate::manifolds::utils::ein_sum;
    ...
    #[test]
    fn small_einsum() {
        let mat = create_test_matrix::<NdArray>(3, 3, vec![
            3.0,4.0,5.0,
            6.0,7.0,3.0,
            -10.0,-4.0,-1.0,
        ]);
